...
const Client = module.exports = function (ufo, options) {
...
Client.prototype._commandMode = function (callback) {
  ...
  // Say hello.
  this._sendAndWait(this._options.password || UdpUtils.assembleCommand('hello'), (err, msg) => {
    if (err) {
      ...
      const ufo = UdpUtils.parseHelloResponse(msg);
...
Client.prototype._endCommand = function (callback) {
  if (this._dead) return;
  this._send(UdpUtils.assembleCommand('endCmd'), (err) => {
    if (err) this._socket.emit('error', err);
    else callback();
  });
};
...
Client.prototype._sendAndRequire = function (cmd, expected, callback) {
  this._sendAndVerify(cmd, expected, (err, resp, matches) => {
...
Client.prototype._sendAndVerify = function (cmd, expected, callback) {
  this._sendAndWait(cmd, (err, resp) => {
    let matches = false;
    if (!err) {
...
Client.prototype._sendAndWait = function (cmd, callback) {
  ...
  this._socket.send(cmd.send, UdpUtils.getDefaultPort(), this._options.host, (err) => {
...
Client.prototype.connect = function (callback) {
  if (this._dead) return;
  const port = this._options.udpPort;
  if (port >= 0) {
...
Client.prototype.version = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('moduleVersion'), (err, version) => {
...
Client.prototype.reboot = function (callback) {
  ...
  this._commandMode(() => {
    this._send(UdpUtils.assembleCommand('reboot'), (err) => {
      if (err) this._socket.emit('error', err);
...
Client.prototype.factoryReset = function (callback) {
  ...
  this._commandMode(() => {
    ...
    this._sendAndRequire(cmd, expected, (msg) => {
...
Client.prototype.getNtpServer = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('ntp'), (err, ipAddress) => {
...
Client.prototype.setNtpServer = function (ipAddress, callback) {
  ...
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('ntp', ipAddress), (err) => {
...
Client.prototype.getUdpPassword = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('udpPassword'), (err, password) => {
...
Client.prototype.setUdpPassword = function (password, callback) {
  ...
  if (typeof callback === 'function') this._ufo._disconnectCallback = callback;
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('udpPassword', password), (err) => {
      if (err) this._socket.emit('error', err);
      else this._ufo.disconnect();
    });
  });
...
Client.prototype.getTcpPort = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('tcpServer'), (err, tcpServer) => {
  ...
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('tcpServer'), (err, tcpServer) => {
      if (err) callback(err);
        ...
        this._sendAndWait(UdpUtils.assembleCommand('tcpServer', tcpServer[0], tcpServer[1], port, tcpServer[3]), (err) => {
          if (err) this._socket.emit('error', err);
          else this._ufo.disconnect();
        });
      }
...
Client.prototype.getWifiAutoSwitch = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiAutoSwitch'), (err, value) => {
...
Client.prototype.setWifiAutoSwitch = function (value, callback) {
  ...
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiAutoSwitch', value), (err) => {
      this._endCommand(function (err) { this(err); }.bind(callback, err));
    });
  });
};
...
Client.prototype.getWifiMode = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiMode'), (err, mode) => {
...
Client.prototype.setWifiMode = function (mode, callback) {
  ...
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiMode', mode), (err) => {
      this._endCommand(function (err) { this(err); }.bind(callback, err));
    });
  });
};
...
Client.prototype.doWifiScan = function (callback) {
  ...
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiScan'), (err, result) => {
      if (!errorReceived) {
        if (err) {
          errorReceived = true;
...
Client.prototype.getWifiApIp = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiApIp'), (err, result) => {
...
Client.prototype.setWifiApIp = function (ip, mask, callback) {
  ...
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiApIp', ip, mask), (err) => {
      this._endCommand(function (err) { this(err); }.bind(callback, err));
    });
  });
};
...
Client.prototype.getWifiApBroadcast = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiApBroadcast'), (err, result) => {
      this._endCommand(function (err, result) {
        let mode;
        switch (result[0]) {
          case '11b':
        ...
        const ssid = result[1];
        const channel = parseInt(result[2].substring(2));
        this(err, {
          mode,
          ssid,
          channel,
...
Client.prototype.setWifiApBroadcast = function (mode, ssid, channel, callback) {
  ...
  let channelError = isNaN(intChannel);
  ...
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiApBroadcast', `11${mode.toUpperCase()}`, ssid, `CH${channel}`), (err) => {
      this._endCommand(function (err) { this(err); }.bind(callback, err));
    });
  });
};
...
Client.prototype.getWifiApPassphrase = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiApAuth'), (err, result) => {
      this._endCommand(function (err, result) {
...
Client.prototype.getWifiApLed = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiApLed'), (err, result) => {
      this._endCommand(function (err, result) {
        this(err, result === 'on');
      }.bind(callback, err, result));
    });
...
Client.prototype.setWifiApLed = function (onOff, callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiApLed', onOff ? 'on' : 'off'), (err) => {
      this._endCommand(function (err) { this(err); }.bind(callback, err));
    });
  });
};
...
Client.prototype.getWifiApDhcp = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiApDhcp'), (err, result) => {
      this._endCommand(function (err, result) {
        const dhcp = { on: result[0] === 'on' };
        if (dhcp.on) {
...
Client.prototype.setWifiApDhcp = function (start, end, callback) {
  ...
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiApDhcp', 'on', start, end), (err) => {
      this._endCommand(function (err) { this(err); }.bind(callback, err));
    });
  });
};
...
Client.prototype.disableWifiApDhcp = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiApDhcp', 'off'), (err) => {
      this._endCommand(function (err) { this(err); }.bind(callback, err));
    });
  });
};
...
Client.prototype.getWifiClientApInfo = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiClientApInfo'), (err, result) => {
      this._endCommand(function (err, result) {
        const info = {
          ssid: null,
          mac: null,
        ...
        if (result !== 'Disconnected') {
          const match = result.match(/(.{1,32})\(([0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2})\)/i);
          info.ssid = match[1];
...
Client.prototype.getWifiClientApSignal = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiClientApSignal'), (err, result) => {
      this._endCommand(function (err, result) {
...
Client.prototype.getWifiClientIp = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiClientIp'), (err, result) => {
      this._endCommand(function (err, result) {
        this(err, {
          dhcp: result[0] === 'DHCP',
          ip: result[1],
...
Client.prototype.setWifiClientIpDhcp = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiClientIp', 'DHCP'), (err) => {
      this._endCommand(function (err) { this(err); }.bind(callback, err));
    });
  });
};
...
Client.prototype.setWifiClientIpStatic = function (ip, mask, gateway, callback) {
  ...
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiClientIp', 'static', ip, mask, gateway), (err) => {
      this._endCommand(function (err) { this(err); }.bind(callback, err));
    });
  });
};
...
Client.prototype.getWifiClientSsid = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiClientSsid'), (err, result) => {
      this._endCommand(function (err, result) {
        this(err, result);
      }.bind(callback, err, result));
    });
...
Client.prototype.setWifiClientSsid = function (ssid, callback) {
  ...
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiClientSsid', ssid), (err) => {
      this._endCommand(function (err) { this(err); }.bind(callback, err));
    });
  });
};
...
Client.prototype.getWifiClientAuth = function (callback) {
  this._commandMode(() => {
    this._sendAndWait(UdpUtils.assembleCommand('wifiClientAuth'), (err, result) => {
      this._endCommand(function (err, result) {
        this(err, {
          auth: result[0],
          encryption: result[1],
